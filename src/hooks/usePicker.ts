...
export function usePicker(props: WheelPickerProps) {
  ...
  const configs = useMemo<Required<DatePickerConfig>>(() => {
    ...
    if (config.month && !config.month.formatter) {
      config.month.formatter = (value) => jalaliMonths[value.month! as keyof typeof jalaliMonths];
    }
  ...
  function checkDateIsHoliday(dayOfYear: number): boolean {
    return solarEvents[dayOfYear as keyof typeof solarEvents]?.holiday || false;
  }
  ...
  function addExtraDateInfo(
    ...
    targetSelectedDate.monthText = jalaliMonths[targetSelectedDate.month! as keyof typeof jalaliMonths];
    ...
    targetSelectedDate.isHoliday =
      solarEvents[targetSelectedDate.dayOfYear as keyof typeof solarEvents]?.holiday ?? false;
