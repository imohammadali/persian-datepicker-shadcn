...
export function convertSelectedDateToObject(
  ...
  selectedDate.forEach((v) => {
    const [type, value] = v.split('-') as [keyof PickerDateModel, string];
...
export function convertSelectedDateObjectToArray(
  ...
  const result = Object.keys(selectedDate).map((type) => ({
    type,
    value: selectedDate[type as keyof PickerDateModel],
  }));

  const sortedColumnsOrder = result.sort((columnA, columnB) =>
    columnsSortOrder[columnA.type as keyof typeof columnsSortOrder] > columnsSortOrder[columnB.type as keyof typeof columnsSortOrder] ? 1 : 0,
  );
